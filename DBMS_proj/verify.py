...
def verify_admin(email, password):
	cursor = db.cursor()
	cursor.execute('SELECT pass FROM clinics WHERE email = %s AND pass = %s', (email,password))
	results = cursor.fetchall() #gives a list of rows(tuples)
	cursor.close()
	if len(results) == 0:
		#print('list empty')
...
def reg_admin(name, email, mobile, password):
	cursor = db.cursor()
	sql = 'INSERT INTO clinics (name, email, mobile, pass) VALUES (%s, %s, %s, %s)'
	values = [name, email, mobile, password]
	cursor.execute(sql, values)
	db.commit()
	cursor.close()